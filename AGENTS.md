...
- **For any dataset-improvement task** (covering a period/region better, adding civilisations/peoples/empires/events, or raising the spatial/temporal resolution of existing entities), follow the `improve-data` skill (`.Codex/skills/improve-data/SKILL.md`) — it covers the survey → research → author → validate workflow, sourcing rules, and gotchas. The `verify` skill (`.Codex/skills/verify/SKILL.md`) covers visually checking the result in the running app.
